...
Если этот запрос на старте падает (временный sudo/z2r-not-ready на
только что поднятом сервере), бот всё равно подключается к Discord, но
шлёт владельцу (`OWNER_USER_ID`) личное сообщение с настоящей причиной
сбоя вместо того, чтобы молча остаться с пустым списком. Любая команда,
которой нужны стратегии (`/strategy_list`, `/voice_test`,
`/voice_test_all`, `/voice_rank`), сама пробует перечитать список ещё
раз перед тем, как отказать — `/refresh_strategies` по-прежнему работает
как ручной способ то же самое, но теперь показывает причину, если снова
не получилось.
